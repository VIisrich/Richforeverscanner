...
def set_bot_status(status: bool):
    try:
        ...
        requests.put(JSONBIN_URL, json=current, headers={"X-Master-Key": JSONBIN_MASTER_KEY, "Content-Type": "application/json"}, timeout=3)
...
# ==============================================================================
st.sidebar.title("⚡ RICHFOREVER AI")
st.sidebar.markdown("---")

page = st.sidebar.radio("Navigation", ["Home / Dashboard", "Single-Shot Analysis", "Multi-Timeframe Confluence"])
...
if page == "Home / Dashboard":
    ...
    st.markdown("<p style='text-align: center; color: #888;'>ICT Vision Confluence & Live MT5 Telemetry Hub</p>", unsafe_allow_html=True)
    st.markdown("---")
    
    st.markdown("""
    ### Welcome to RichforeverAI
    Use the sidebar navigation to switch between scanning modules:
    * **Single-Shot Analysis**: Upload individual chart screenshots for fast ICT FVG and setup evaluation.
    * **Multi-Timeframe Confluence**: Synthesize Higher Timeframe (H1) macro bias with lower timeframe (5m) entries using multiple chart feeds.
    """)
...
# ==============================================================================
    ...
    st.title("📸 Single Chart Analysis")
    st.markdown("<p style='text-align: center; color: #888;'>ICT Vision Confluence</p>", unsafe_allow_html=True)
    st.markdown("---")
...
# ==============================================================================
    ...
    st.title("🔄 Multi-Timeframe Confluence")
    st.markdown("<p style='text-align: center; color: #888;'>Multi-TF Fusion Scan</p>", unsafe_allow_html=True)
    st.markdown("---")
